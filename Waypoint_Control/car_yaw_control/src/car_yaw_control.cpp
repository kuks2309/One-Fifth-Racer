...
double virtual_cross_track_error = 0;

void cross_track_error_Callback(const std_msgs::Float32& msg)
{
...
int main(int argc, char **argv)
{
	...
	*/ 
	...
	ros::Subscriber sub_cross_track_error              = n.subscribe(cross_track_error_topic,1, cross_track_error_Callback);
  ...
  while(ros::ok())
  {
      ...
      if(control_action_flag == 1)
      {
		...
		{
			steering_angle.data = control_xte();
			yaw_control_steering_output_pub.publish(steering_angle);    
		}
		...
		else if (steering_control_mode == 8)  //traffic cone control
		{
			...
			target_yaw = traffic_cone_target_yaw;
			pid_output = control_yaw();
			steering_angle.data = pid_output;
			yaw_control_steering_output_pub.publish(steering_angle);    
		}
