...
void obstacle_avoid_new_waypoint_Callback(const geometry_msgs::Pose2D &msg)
{
	...
	obstacle_avoid_target_waypoint = msg;
}
...
int main(int argc, char **argv)
{
	...
	{	
		...
		if(run_flag == 1)
		{
			...
			if(lidar_obs_flag == true) ROS_INFO("Obstacle Avoidance Mode!!!");
	  ...
	  //printf("%d %d \n", wp_go_id, no_waypoints);
			...
			{ 		  
				if(lidar_obs_flag == false)
				{
					...
					delta_x = my_waypoints_list[wp_go_id].x - my_waypoints_list[wp_go_id-1].x;   
					delta_y = my_waypoints_list[wp_go_id].y - my_waypoints_list[wp_go_id-1].y;
				}
				else
				{
					my_target_pose_goal_prev.x = my_waypoints_list[wp_go_id-1].x;// - initial_utm_pose.x;
					...
					delta_x = -obstacle_avoid_target_waypoint.y; // - my_waypoints_list[wp_go_id-1].x;   
					delta_y = obstacle_avoid_target_waypoint.x; //- my_waypoints_list[wp_go_id-1].y;
			...
			pos_error_y = abs(my_pose.y - my_waypoints_list[wp_go_id].y);                         // utm 좌표계에서 내위치와 목표 waypoints 간의 Y축 거리
			...
			temp = Draw_Marker_point_avoid_wp(obstacle_avoid_target_waypoint.x, obstacle_avoid_target_waypoint.y);
			...
			{           
				...
				steering_control_mode = 3;
				
				// PID 제어는 실패 함 Fuzzy Controller 로 바꾸어야 할 것 같음 2023.10.2 //
				/*
				
				double x1,y1;
				virtual_cross_track_error = virtual_xte_calculation(wp_go_id,&x1, &y1);
				
				temp = Draw_Marker_point(x1, y1);  // utm 좌표를 입력해야함
				marker_point_pub1.publish(temp);
								
				steering_control_mode_msg.data =  steering_control_mode;
				steering_control_mode_pub.publish(steering_control_mode_msg);
				
				std_msgs::Float32 virtual_cross_track_error_msg;
				virtual_cross_track_error_msg.data = virtual_cross_track_error;
				virtual_cross_track_error_pub.publish(virtual_cross_track_error_msg);
				printf("Virtual cross track error %6.3lf\n",virtual_cross_track_error);
				
				//printf(" Steering angle is limited \n"); 
				printf("----------------------------\n");
				*/    
			}
			else
			...
			else
			{
				...
				c_speed.data = -0.1;   // obstacle detection speed (reduce speed)
			}
		...
		{ 
		  ...
		  // Points are blue
			...
			end_p.x =  start_p.x + 6.0*cos(DEG2RAD( car_steering_angle )); 
			end_p.y =  start_p.y + 6.0*sin(DEG2RAD( car_steering_angle )); 
